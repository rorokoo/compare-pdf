...
class ComparePdf {
  ...
  baselinePdfFolder(baselineFolder) {
    if (baselineFolder) {
      const baselinePdfBasePath = path.resolve(baselineFolder)

      if (baselinePdfBasePath) {
        this.config.paths.baselinePdfRootFolder = baselinePdfBasePath
      } else {
        this.result = {
          status: 'failed',
          message: `given baseline path does not exist`,
        }
      }
    } else {
      this.result = {
        status: 'failed',
        message: 'No base directory path set. try again.',
      }
    }
    return this
  }
